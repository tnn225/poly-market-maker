...
def evaluate_model(model, dataset, show_results=True):
    test_df = dataset.test_df.copy()
    
    # # Filter by bid=0.01
    # test_df = test_df[test_df['bid'] <= 0.01].copy()
    # print(f"Filtered test_df to bid=0.01: {len(test_df)} rows")
    ...
    test_df['seconds_left_bucket'] = pd.qcut(test_df['seconds_left'], 10, duplicates='drop')
    ...
    for (d_bucket, t_bucket), group in test_df.groupby(['delta_bucket', 'seconds_left_bucket'], observed=False):
        ...
        group_results.append({
            ...
            'opportunity': round(group['is_up'].sum() - group['bid'].sum(), 3),
            ...
            'num_rows': len(group),
            'pnl': round(buy_trades['pnl'].sum(), 3),  
        })
    ...
    results_df = results_df.sort_values(['seconds_left_bucket', 'num_intervals'], ascending=[False, True])
...
def main():
    ...
    MODELS = {
        # "LogisticRegression": Model("LogisticRegression", LogisticRegression(max_iter=10000, C=0.5, solver='lbfgs'), feature_cols=FEATURE_COLS),
        "RandomForestClassifier_200": Model("RandomForestClassifier_200", RandomForestClassifier(n_estimators=200, max_depth=10, min_samples_split=50, min_samples_leaf=50, max_features='log2', bootstrap=False, class_weight='balanced_subsample', random_state=42), feature_cols=FEATURE_COLS, dataset=dataset),
    }

    model = MODELS["RandomForestClassifier_200"]
    evaluate_model(model, dataset, show_results=True)
...
def calibrate_model(rf_model):
    ...
    models = [
        ...
        # Model("BucketClassifier_features_{len(feature_cols)}", BucketClassifier(), feature_cols=feature_cols, dataset=dataset),
        # Model("BidClassifier_features_{len(feature_cols)}", BidClassifier(), feature_cols=feature_cols, dataset=dataset),
    ]
